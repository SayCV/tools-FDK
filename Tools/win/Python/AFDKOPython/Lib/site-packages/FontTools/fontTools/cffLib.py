...
class CharsetCompiler(object):
	...
	def __init__(self, strings, charset, parent):
		...
		data1 = packCharset1(charset, isCID, strings)
		data2 = packCharset2(charset, isCID, strings)
...
def packCharset2(charset, isCID, strings):
	fmt = 2
	ranges = []
	first = None
	end = 0
	...
	for first, nLeft in ranges:
		data.append(packCard16(first) + packCard16(nLeft))
	return bytesjoin(data)


def packCharset1(charset, isCID, strings):
	fmt = 1
	ranges = []
	first = None
	end = 0
	...
	lastChar = getNameID(charset[-1], strings)
	for name in charset[1:]:
		SID = getNameID(name, strings)
		if first is None:
			first = SID
		...
		if nLeft == 255:
			ranges.append((first, nLeft))
			if lastChar == end:
				end = 0 # We've written the last range. We don't want to execute the if end: below.
			else:
				first = None
	...
	data = [packCard8(fmt)]
	for first, nLeft in ranges:
		data.append(packCard16(first) + packCard8(nLeft))
	return bytesjoin(data)
