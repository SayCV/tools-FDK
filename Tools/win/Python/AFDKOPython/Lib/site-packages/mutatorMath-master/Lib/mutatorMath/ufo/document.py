...
"""
    ...
    Read and write mutator math designspaces for fonts to XML. 
    ...
    A DesignSpaceDocumentReader object can buse

"""

def newLogger(proposedLogPath):
    """ Create a new logging object at this path """
    logging.basicConfig(filename=proposedLogPath, level=logging.INFO, format='%(asctime)s MutatorMath %(message)s')
...
class DesignSpaceDocumentWriter(object):
    ...
    *   toolVersion: version of this tool
    ...
    def __init__(self, path, toolVersion=3, verbose=False, logPath=None):
        self.path = path
        ...
        if logPath is None:
            logPath = os.path.join(os.path.dirname(path), "mutatorMath.log")
        self.logger = newLogger(logPath)
        if self.verbose:
            self.logger.info("Writing designspace document: %s", path)
    ...
    def save(self):
        ...
        if self.verbose:
            self.logger.info("Saved to %s", self.path)
    ...
    def _makeMasterElement(self, source=None, location=None, glyphname=None):
        """ Make a masterElement. """
        #masterElement = self.doc.createElement('master')
        #if source is not None:
        #    masterElement.setAttribute('source', source)
        #if location is not None:
        #    self.addLocationToElement(masterElement, location)
        #if glyphname is not None:
        #    masterElement.setAttribute('glyphname', glyphname)
...
class DesignSpaceDocumentReader(object):
    ...
    def __init__(self, documentPath, ufoVersion, roundGeometry=False, verbose=False, logPath=None):
        ...
        self.logger = newLogger(logPath)
        if self.verbose:
            self.logger.info("Building designspace document: %s", documentPath)
        ...
        for sourceElement in self.root.findall(".sources/source"):
            ...
            for libElement in sourceElement.findall('.lib'):
                if libElement.attrib.get('copy') == '1':
                    if self.libSource is not None:
                        ...
                        if self.verbose:
                            self.logger.info("%s is lib source", sourceName)
            ...
            for groupsElement in sourceElement.findall('.groups'):
                if groupsElement.attrib.get('copy') == '1':
                    if self.groupsSource is not None:
                        ...
                        if self.verbose:
                            self.logger.info("%s is groups source", sourceName)
            ...
            for infoElement in sourceElement.findall(".info"):
                if infoElement.attrib.get('copy') == '1':
                    if self.infoSource is not None:
                        ...
                        if self.verbose:
                            self.logger.info("Copy info from %s.", sourceName)
                if infoElement.attrib.get('mute') == '1':
                    self.muted['info'].append(sourceName)
                    if self.verbose:
                        self.logger.info("Math info from %s muted.", sourceName)
            ...
            for kerningElement in sourceElement.findall(".kerning"):
                if kerningElement.attrib.get('mute') == '1':
                    ...
                    if self.verbose:
                        self.logger.info("Math kerning from %s muted.", sourceName)
    ...
    def _readSingleInstanceElement(self, instanceElement, makeGlyphs=True, makeKerning=True, makeInfo=True):
        ...
        if self.verbose:
            self.logger.info("processing %s at %s", filename, instancePath)
